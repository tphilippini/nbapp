...
async function findTodayMatches(date) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const uri = `https://cdn.nba.com/static/json/liveData/scoreboard/todaysScoreboard_00.json`;
      const response = await fetch(uri);
      const matches = await response.json();
      resolve(matches.scoreboard.games);
    } catch (error) {
      console.log(error);
      reject(error);
    }
...
async function checkBoxScore(dateFormatted, gameId) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const uri = `https://cdn.nba.com/static/json/liveData/boxscore/boxscore_${gameId}.json`;
      const response = await fetch(uri);
      const boxscore = await response.json();
      resolve(boxscore.game);
    } catch (error) {
      console.log(error);
      reject(error);
    }
...
export { findTodayMatches, checkBoxScore };
