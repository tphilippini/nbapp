...
import Devices from "../devices/device.model.js";
...
authController.post = (req, res) => {
        ...
        if (!email || !password || !userType) {
          ...
          if (errors.length === 0) {
            passport.authenticate("local", { session: false }, (err, user) => {
              if (err) {
                console.error("Hi! Password validation on error...");
                errors.push(err);
                return checkEvent.emit("error", errors);
              }
              console.log("Hi! Generating tokens...");
  ...
  checkEvent.on("success_password_grant", async (result) => {
    ...
    if (existingDevice) {
      console.log("Hi! Updating existing device...");
      ...
      console.log("Hi! Creating new device...");
...
authController.reset = (req, res) => {
  ...
  const checking = () => {
    ...
    if (!grantType) {
      ...
      if (grantType === "reset") {
        console.log("Hi! Reseting password...");
        ...
        if (!token || !userType || !password || !confirmPassword) {
          ...
          if (errors.length === 0) {
            console.log("Hi! Validating token...");
...
authController.forgot = (req, res) => {
  ...
  const checking = () => {
    ...
    if (!grantType) {
      ...
      if (grantType === "forgot") {
        console.log("Hi! Send reset link password...");
